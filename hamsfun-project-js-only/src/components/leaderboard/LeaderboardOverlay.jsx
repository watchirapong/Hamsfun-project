...
/**
 * Leaderboard Overlay Component with performance optimizations
 * Optimized for 10,000+ concurrent users
 */
const USERS_PER_PAGE = 100;

export const LeaderboardOverlay = React.memo(({
  houseLeaderboard,
  theme,
  onClose,
  onFetchMembers,
  ...
  const totalPages = useMemo(() => Math.ceil(houseLeaderboard.length / USERS_PER_PAGE), [houseLeaderboard.length]);
  const startIndex = useMemo(() => (currentPage - 1) * USERS_PER_PAGE, [currentPage]);
  const endIndex = useMemo(() => startIndex + USERS_PER_PAGE, [startIndex]);
  const currentPageHouses = useMemo(() => houseLeaderboard.slice(startIndex, endIndex), [houseLeaderboard, startIndex, endIndex]);
  const pageRange = useMemo(() => `${startIndex + 1}-${Math.min(endIndex, houseLeaderboard.length)}`, [startIndex, endIndex, houseLeaderboard.length]);
