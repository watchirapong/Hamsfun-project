...
/**
 * Profile Section Component with performance optimizations
...
export const ProfileSection = React.memo(({
  ...
  const handleRankUpClick = useCallback((e) => {
    e.stopPropagation(); // Prevent card flip
    onRankUp();
  }, [onRankUp]);

  const rankProgressPercentage = useMemo(() => {
    return user.nextRankPoints 
      ? Math.min((user.rankPoints / user.nextRankPoints) * 100, 100) 
      : 100;
  }, [user.rankPoints, user.nextRankPoints]);

  const canRankUpValue = useMemo(() => canRankUp(), [canRankUp]);
