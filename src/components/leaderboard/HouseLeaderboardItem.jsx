...
/**
 * House Leaderboard Item Component with performance optimizations
 * Optimized for 10,000+ concurrent users
 */
export const HouseLeaderboardItemComponent = React.memo(({ 
  item, 
  onFetchMembers,
  theme,
  ...
  const [memberRankMap, setMemberRankMap] = useState(new Map());
  const previousRankMapRef = useRef(new Map());
  ...
  const isTopThree = useMemo(() => item.rank <= 3, [item.rank]);

  // Sort members by rank (S->G), then alphabetically by username
  const sortMembersByRank = useCallback((membersList) => {
  ...
  }, [isExpanded, members, onFetchMembers, item.houseId, sortMembersByRank]);

  // Check if a member's rank has increased (for animation)
  const shouldAnimateMember = useCallback((member) => {
    ...
    return getRankPriority(currentRank) > getRankPriority(previousRank);
  }, [currentUserDiscordUsername]);
...
HouseLeaderboardItemComponent.displayName = 'HouseLeaderboardItemComponent';
