...
/**
 * Reward Animation Component with performance optimizations
 * Optimized for 10,000+ concurrent users
 */
export const RewardAnimation = React.memo(({ animation, theme }) => {
  ...
  const animationDuration = 7000; // 7 seconds total for slower float
  const targetHeight = typeof window !== 'undefined' ? window.innerHeight * 0.8 : 600; // 80% of screen height
  ...
  }, [isPopping, shouldBurst]);
  
  // Easing function for smooth float
  const easeOutCubic = React.useCallback((t) => {
  ...
  }, [animation.startTime, animation.forceBurst, animation.driftX, animation.x, isPopping, shouldBurst, popStartTime, targetHeight, easeOutCubic]);
  ...
  }, [animation.type, animation.itemIcon, animation.itemName]);
  ...
  }, [animation.type, animation.value, animation.itemName]);
  ...
  const bottomPosition = typeof window !== 'undefined' ? (window.innerHeight - animation.y) + Math.abs(currentPosition.y) : 0;
...
RewardAnimation.displayName = 'RewardAnimation';
