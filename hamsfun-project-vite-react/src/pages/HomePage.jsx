...
const HomePage = () => {
  ...
  const processingObjectives = useRef(new Set());
  
  // Ref to track if quest panel should animate (only on manual open)
  const questPanelShouldAnimate = useRef(false);
    ...
    if (isAuthenticated) {
      ...
      if (token) {
        const socket = socketService.connect({
          url: import.meta.env.VITE_SOCKET_URL || 'https://api.questcity.cloud',
          path: import.meta.env.VITE_SOCKET_PATH || '/hamster-world-api/socket.io',
          token: token
        });
  ...
  const handleFetchHouseMembers = useCallback(async (houseId) => {
    try {
            ...
            <button
              onClick={() => {
                try {
                  ...
                  const redirectUri = `${import.meta.env.VITE_APP_URL || window.location.origin}/auth/handover`;
