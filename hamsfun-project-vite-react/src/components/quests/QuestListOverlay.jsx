...
/**
 * Quest List Overlay Component with performance optimizations
 * Optimized for 10,000+ concurrent users
 */
export const QuestListOverlay = React.memo(({
  questsState,
  theme,
  showQuestOverlay,
  setShowQuestOverlay,
  ...
  const uncompletedQuests = useMemo(() => questsState
  ...
  const completedQuests = useMemo(() => questsState
    .filter(q => isQuestTrulyCompleted(q))
    .sort((a, b) => {
      // Main Quests first
      const aIsMain = a.type === "Main Quest";
  ...
  const getRewardDisplay = useCallback((reward) => {
    ...
    const formatRange = (min, max, value) => {
  ...
  }, [dragY, setShowQuestOverlay, setSelectedQuestId]);
  ...
  const canScrollContent = useCallback((direction) => {
    if (!scrollableContentRef.current) return false;
  ...
  }, [showQuestOverlay, handleMouseDown, handleMouseMove, handleMouseUp]);
  
  // Helper function for reward rendering in completion section
  const renderReward = useCallback((reward, index, itemDetailsCache) => {
    const formatRange = (min, max, value) => {
      if (min !== undefined && max !== undefined && min !== max) {
        return `${formatShortNumber(min)} - ${formatShortNumber(max)}`;
      } else if (value !== undefined) {
                        ...
                        >
                          ...
                          <div className={`flex justify-center gap-6 ${quest.rewardSubmissionStatus === 'pending' ? 'opacity-30' : ''}`}>
                            {quest.rewards.filter(reward => reward.type !== 'skill' && reward.type !== 'leaderboard').map((reward, index) => 
                              renderReward(reward, index, itemDetailsCache)
                            )}
                          </div>
                        </button>
                      </div>
...
QuestListOverlay.displayName = 'QuestListOverlay';
