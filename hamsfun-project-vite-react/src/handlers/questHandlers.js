/**
 * Quest Handlers with performance optimizations
 ...
 */
export const useQuestHandlers = (params) => {
  const {
    questsState,
    setQuestsState,
    setSelectedQuestId,
  ...
  const handleQuestCardClick = (questId) => {
  ...
  const handleObjectiveClick = (questId, objectiveIndex) => {
  ...
  const handleSubmitImage = async () => {
    ...
    try {
      ...
      if (typeof uploadedImage === 'string' && uploadedImage.startsWith('data:')) {
        ...
        try {
          formData.append('imageProof', uploadedImage);
        } catch (e) {
      ...
      const submitResponse = await questAPI.submitQuest(quest.id.toString(), formData);
      ...
      if (hasGrantedRewards) {
        ...
        if (!awardedRewards.has(rewardKey)) {
          ...
          if (rewardsToProcess && rewardsToProcess.items && Array.isArray(rewardsToProcess.items)) {
            rewardsToProcess.items.forEach((item) => {
              if (item.quantity > 0) {
                awardObjectiveReward({
                  type: 'item',
                  value: item.quantity,
  ...
  const handleApproveObjective = useCallback((questId, objectiveIndex) => {
  ...
  const handleApproveReward = useCallback((questId) => {
